...
ALTER TABLE receitas ADD PRIMARY KEY (codigo_paciente);
...
ALTER TABLE `dados_clinica` CHANGE `banco` `banco1` VARCHAR( 50 ) NULL DEFAULT NULL ,
    CHANGE `agencia` `agencia1` VARCHAR( 15 ) NULL DEFAULT NULL ,
    CHANGE `conta` `conta1` VARCHAR( 15 ) NULL DEFAULT NULL ;
  
ALTER TABLE `dados_clinica` ADD `banco2` VARCHAR( 50 ) NULL DEFAULT NULL AFTER `conta1` ,
    ADD `agencia2` VARCHAR( 15 ) NULL DEFAULT NULL AFTER `banco2` ,
    ADD `conta2` VARCHAR( 15 ) NULL DEFAULT NULL AFTER `agencia2` ;
...
CREATE VIEW v_orcamento AS (
SELECT tpo.codigo_orcamento AS codigo_orcamento, tor.parcelas AS parcelas, tor.confirmado AS confirmado, tor.baixa AS baixa, tpo.codigo AS codigo_parcela, tpo.datavencimento AS data, tpo.valor AS valor, tpo.pago AS pago, tpo.datapgto AS datapgto, tp.codigo AS codigo_paciente, tp.nome AS paciente, td.nome AS dentista, td.sexo AS sexo_dentista
FROM parcelas_orcamento tpo
INNER JOIN orcamento tor
ON tpo.codigo_orcamento = tor.codigo
INNER JOIN pacientes tp
ON tor.codigo_paciente = tp.codigo
JOIN dentistas td
ON tor.cpf_dentista = td.cpf
);
