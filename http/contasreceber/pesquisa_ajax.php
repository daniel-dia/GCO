...
<?php
    ...
    $sql = "SELECT * FROM `contasreceber` WHERE " . $where . " ORDER BY `datavencimento` ASC, `codigo` ASC";
...
?>
    <tr bgcolor="#<?php echo $odev?>" onmouseout="style.background='#<?php echo $odev?>'" onmouseover="style.background='#DDE1E6'">
      ...
      <td width="21%" align="right"><input type="text" size="13" name="datapagamento" id="datapagamento" value="<?php echo converte_data($conta->RetornaDados('datapagamento'), 2)?>" onblur="Ajax('contasreceber/atualiza', 'conta_atualiza', 'codigo=<?php echo $conta->RetornaDados('codigo')?>&datapagamento='%2Bthis.value)" onKeypress="return Ajusta_Data(this, event);" <?php echo ((!verifica_nivel('contas_receber', 'E'))?'disabled':'')?>></td>
      <td width="5%" align="center"><?php echo ((verifica_nivel('contas_receber', 'A'))?'<a href="javascript:Ajax(\'contasreceber/extrato\', \'conteudo\', \'codigo='.$conta->RetornaDados('codigo').'" onclick="return confirmLink(this)"><img src="imagens/icones/excluir.gif" alt="Excluir" width="19" height="19" border="0"></a>':'')?></td>
    </tr>
<?php
	}
	if($odev == $impar) {
