...
<?php
        ...
        $sql = "SELECT * FROM `contaspagar_dent` WHERE " . $where . " `codigo_dentista` = '" . $codigo_dentista. "' ORDER BY `datavencimento` ASC, `codigo` ASC";
...
?>
    <tr bgcolor="#<?php echo $odev?>" onmouseout="style.background='#<?php echo $odev?>'" onmouseover="style.background='#DDE1E6'">
      ...
      <td width="21%" align="right"><input type="text" size="13" name="datapagamento" id="datapagamento" value="<?php echo converte_data($conta->RetornaDados('datapagamento'), 2)?>" onblur="Ajax('contaspagar_dent/atualiza', 'conta_atualiza', 'codigo=<?php echo $conta->RetornaDados('codigo')?>&datapagamento='%2Bthis.value)" onKeypress="return Ajusta_Data(this, event);" <?php echo ((!verifica_nivel('contas_pagar', 'E'))?'disabled':'')?>></td>
      <td width="5%" align="center"><?php echo ((verifica_nivel('contas_pagar', 'A'))?'<a href="javascript:Ajax(\'contaspagar_dent/extrato\', \'conteudo\', \'codigo='.$conta->RetornaDados('codigo').'&codigo_dentista='.$_GET['codigo_dentista'].'&senha_dentista='.$_GET['senha_dentista'].'" onclick="return confirmLink(this)"><img src="imagens/icones/excluir.png" alt="Excluir" width="19" height="19" border="0"></a>':'')?></td>
    </tr>
<?php
	}
	if($odev == $impar) {
